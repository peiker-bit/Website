...
import { LogOut, Mail, Settings, LayoutDashboard, User, Menu, X, Calendar, UserPlus, Briefcase } from 'lucide-react';
...
const AdminLayout = ({ children }) => {
  ...
  const navigationItems = [
    ...
    { path: '/admin/neumandanten', icon: <UserPlus size={20} />, label: 'Neumandanten' },
    { path: '/admin/jobs', icon: <Briefcase size={20} />, label: 'Stellenangebote' },
        ...
        <nav className="sidebar-nav">
          {navigationItems.map((item) => (
            <Link
              key={item.path}
              to={item.path}
              className={`nav-item ${(item.exact ? location.pathname === item.path : location.pathname.startsWith(item.path)) ? 'active' : ''}`}
